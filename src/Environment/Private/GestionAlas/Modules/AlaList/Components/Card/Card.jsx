    ...
    useEffect(() => {
        const datosFiltrados = rows.filter((row) => {
          return (
            ...
            row.id_torre.toString().toLowerCase().includes(filtro.toLowerCase())||
              ...
              <Table sx={{ minWidth: 650 }} aria-label="simple table">
                ...
                <TableBody>
                  {datosFiltrados.map((fila) => (
                    <TableRow key={fila.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }} onClick={()=>editItem(fila.id)}  style={{backgroundColor:IdEditGestion==fila.id?'lightgrey':''}}>
