    ...
    useEffect(() => {
        const datosFiltrados = rows.filter((row) => {
          return (
            ...
            row.username.toString().toLowerCase().includes(filtro.toLowerCase())||
            row.cedula.toString().toLowerCase().includes(filtro.toLowerCase())||
              ...
              <Table  aria-label="simple table">
                ...
                <TableBody>
                  {datosFiltrados.map((fila) => (
                    <TableRow key={fila.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }} onClick={()=>editItem(fila.id)} style={{backgroundColor:IdEditGestion==fila.id?'lightgray':''}}>
                      {/* <TableCell component="th" scope="row">
                        {fila.id}
                      </TableCell> */}
                      {/* <TableCell>{fila.status}</TableCell> */}
                      <TableCell>{fila.username}</TableCell>
